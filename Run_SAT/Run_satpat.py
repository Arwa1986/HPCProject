...
def main():
    ...
    training_neg_walks, test_neg_walks = split_into_evaluation_and_training_lists(neg_walks, 66, seed_value)
    write_walks_to_file(training_pos_walks, training_neg_walks, test_pos_walks, test_neg_walks,
                        f'{directory}/{filename}_{trails}_{walks_size}_traces.txt')
    # Build APTA
    apta = Tree()
    ...
    save_Exploration_map_to_file(edsm.hard_patterns,
                                 f'{directory}/{filename}_{trails}_{walks_size}_{learning_strategy}_hard_map.txt')
