...
def main():
    learning_strategy = 'BiasedSiccoSATPAT'
    ...
    training_pos_walks, training_neg_walks, test_pos_walks, test_neg_walks = retrieve_traces(
        f'Traces/{args.filename}_{args.trails}_{args.walks_size}_classicalEDSM_traces.txt')
    # Build APTA
    apta = Tree()
    ...
    save_Exploration_map_to_file(edsm.hard_patterns, f'{args.directory}/{args.filename}_{args.trails}_{args.walks_size}_{learning_strategy}_hard_map.txt')
    save_Exploration_map_to_file(edsm.soft_patterns, f'{args.directory}/{args.filename}_{args.trails}_{args.walks_size}_{learning_strategy}_soft_map.txt')
    ...
    edsm.write_statistics(f'{args.directory}/{args.filename}_{args.trails}_{args.walks_size}_{learning_strategy}_statistics.txt')
    write_to_file_in_new_line(f'{args.directory}/{args.filename}_{args.trails}_{args.walks_size}_{learning_strategy}_statistics.txt',
    ...
    dfasat_graph = sat_solver.run_dfasat_with_patterns(f'{args.directory}/{args.filename}_{args.trails}_{args.walks_size}_{learning_strategy}', edsm.hard_patterns)
